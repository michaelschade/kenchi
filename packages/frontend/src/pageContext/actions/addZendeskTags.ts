...
const addZendeskTags: PageActionRunner<'addZendeskTags'> = async (
  { messageRouter },
  { tags }
) => {
  try {
  ...
  const resp = await messageRouter.sendCommand('pageScript', 'zendeskAddTags', {
    tags,
  });
  if (resp.success) {
